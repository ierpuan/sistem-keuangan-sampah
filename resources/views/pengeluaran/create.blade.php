...
<div class="mb-6">
    ...
    <p class="text-gray-600">Catat pengeluaran operasional</p>
</div>
...
<div class="bg-white rounded-lg shadow p-6">
    <form method="POST" action="{{ route('pengeluaran.store') }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="kategori" value="{{ old('kategori') }}"
                       class="w-full border rounded-lg px-4 py-2 @error('kategori') border-red-500 @enderror"
                       placeholder="Contoh: Listrik, Maintenance, Gaji, dll" required>
                @error('kategori')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="keterangan" rows="3"
                          class="w-full border rounded-lg px-4 py-2 @error('keterangan') border-red-500 @enderror"
                          placeholder="Detail pengeluaran...">{{ old('keterangan') }}</textarea>
                @error('keterangan')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mt-6 flex gap-4">
            ...
            <a href="{{ route('pengeluaran.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-2 rounded-lg">
