        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <div id="kategoriDropdown"
                            ...
                            <a href="{{ route('pengeluaran.index', request()->except('kategori')) }}"
                               class="block px-3 py-2 text-sm hover:bg-gray-100">
                                Semua Kategori
                            </a>

                            @foreach($kategoriList as $kat)
                                <a href="{{ route('pengeluaran.index', array_merge(request()->all(), ['kategori' => $kat])) }}"
                                   class="block px-3 py-2 text-sm hover:bg-gray-100">
                                    {{ $kat }}
                                </a>
                            @endforeach
                    ...
                    <div class="relative">
                        <button type="button"
                            ...
                            class="w-full bg-white border border-gray-300 rounded px-3 py-1.5 text-sm
                                   flex justify-between items-center hover:bg-gray-50">
                            <span>
                                @if(request('periode') === 'hari_ini') Hari Ini
                                @elseif(request('periode') === 'minggu_ini') Minggu Ini
                                @elseif(request('periode') === 'bulan_ini') Bulan Ini
                                @elseif(request('periode') === 'bulan_lalu') Bulan Lalu
                                @elseif(request('periode') === '3_bulan') 3 Bulan Terakhir
                                @elseif(request('periode') === 'tahun_ini') Tahun Ini
                                @else Semua Periode
                                @endif
                            </span>
                            <i class="fas fa-chevron-down text-xs text-gray-400"></i>
                        ...
                        <div id="periodeDropdown"
                             class="hidden absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded shadow-md max-h-56 overflow-auto">
                            <a href="{{ route('pengeluaran.index', request()->except('periode')) }}"
                               class="block px-3 py-2 text-sm hover:bg-gray-100">
                                Semua Periode
                            </a>
                            <a href="{{ route('pengeluaran.index', array_merge(request()->all(), ['periode' => 'hari_ini'])) }}"
                               class="block px-3 py-2 text-sm hover:bg-gray-100">
                                Hari Ini
                            </a>
                            <a href="{{ route('pengeluaran.index', array_merge(request()->all(), ['periode' => 'minggu_ini'])) }}"
                               class="block px-3 py-2 text-sm hover:bg-gray-100">
                                Minggu Ini
                            </a>
                            <a href="{{ route('pengeluaran.index', array_merge(request()->all(), ['periode' => 'bulan_ini'])) }}"
                               class="block px-3 py-2 text-sm hover:bg-gray-100">
                                Bulan Ini
                            </a>
                            <a href="{{ route('pengeluaran.index', array_merge(request()->all(), ['periode' => 'bulan_lalu'])) }}"
                               class="block px-3 py-2 text-sm hover:bg-gray-100">
                                Bulan Lalu
                            </a>
                            <a href="{{ route('pengeluaran.index', array_merge(request()->all(), ['periode' => '3_bulan'])) }}"
                               class="block px-3 py-2 text-sm hover:bg-gray-100">
                                3 Bulan Terakhir
                            </a>
                            <a href="{{ route('pengeluaran.index', array_merge(request()->all(), ['periode' => 'tahun_ini'])) }}"
                               class="block px-3 py-2 text-sm hover:bg-gray-100">
                                Tahun Ini
                            </a>
                        </div>
                ...
                <div class="flex items-end gap-2 sm:col-span-2 lg:col-span-1">
                    <a href="{{ route('pengeluaran.index') }}"
                       class="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-800 px-3 py-1.5 rounded text-sm transition text-center">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gradient-to-r from-gray-700 to-gray-800">
                <tr>
                    ...
                    <th class="px-3 py-3 text-left text-xs font-semibold text-white uppercase hidden md:table-cell">Kategori</th>
